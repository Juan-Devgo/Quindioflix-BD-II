...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error: Plan de suscripcion no encontrado');
        RAISE_APPLICATION_ERROR(-20014, 'El plan de suscripcion especificado no existe');
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error en SP_REGISTRAR_CLIENTE: ' || SQLERRM);
...
-- ============================================================
CREATE OR REPLACE PROCEDURE SP_RENOVAR_SUSCRIPCIONES IS
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(
        'Proceso de renovacion completado. Exitosas: ' || v_renovaciones ||
        ', Errores: ' || v_errores
    );
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error general en SP_RENOVAR_SUSCRIPCIONES: ' || SQLERRM);
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Cuenta eliminada exitosamente. ID usuario: ' || p_id_usuario);

EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error en SP_ELIMINAR_CUENTA: ' || SQLERRM);
...
BEGIN
    SP_ELIMINAR_CUENTA(p_id_usuario => 1);
END;
/
*/
