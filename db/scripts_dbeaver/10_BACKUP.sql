...
BEGIN
    ...
    EXECUTE IMMEDIATE 'GRANT READ, WRITE ON DIRECTORY DIR_BACKUP_QUINDIOFLIX TO C_QUINDIOFLIX';
    DBMS_OUTPUT.PUT_LINE('Permisos READ/WRITE otorgados a C_QUINDIOFLIX sobre el DIRECTORY.');
EXCEPTION
    WHEN OTHERS THEN
...
BEGIN
    ...
    RETURNING backup_id INTO p_backup_id;
    ...
    p_estado := CASE v_estado_job
                    WHEN 'COMPLETED'        THEN 'EXITOSO'
                    WHEN 'COMPLETED_EARLY'  THEN 'EXITOSO (PARCIAL)'
                    ELSE 'ERROR: ' || v_estado_job
                END;

    p_mensaje := 'Backup finalizado con estado ' || v_estado_job ||
                 '. Duracion: ' || v_duracion || ' segundos.';
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        -- Registrar error en auditoria si es posible
        BEGIN
            UPDATE BACKUP_LOG
            SET estado   = 'ERROR',
        ...
        p_estado  := 'ERROR';
        p_mensaje := 'Error durante el backup Data Pump: ' || SQLERRM;
...
BEGIN
    ...
    -- ╔═══════════════════════════════════════════════════════════════
    -- ║  QUINDIOFLIX - HISTORIAL DE RESPALDOS (ultimos ' || p_dias || ' dias)
    -- ╚═══════════════════════════════════════════════════════════════
    ...
    FOR r IN (
        ...
        DBMS_OUTPUT.PUT_LINE('  Duracion   : ' || r.duracion_seg || ' seg.');
    ...
    p_total := v_total;
    p_mensaje := 'Total de backups mostrados: ' || v_total;
    DBMS_OUTPUT.PUT_LINE('---------------------------------------------------------------');
    DBMS_OUTPUT.PUT_LINE('  ' || p_mensaje);

EXCEPTION
    WHEN e_no_dba THEN
        p_mensaje := 'Acceso denegado: SP_LISTAR_BACKUPS solo puede ser ejecutado por usuarios con rol DBA o ROL_ADMIN.';
        DBMS_OUTPUT.PUT_LINE(p_mensaje);
        RAISE_APPLICATION_ERROR(
            -20404,
            p_mensaje
    ...
    WHEN OTHERS THEN
        p_mensaje := 'Error al listar backups: ' || SQLERRM;
        DBMS_OUTPUT.PUT_LINE(p_mensaje);
        RAISE_APPLICATION_ERROR(
            -20405,
            p_mensaje
...
BEGIN
    ...
    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM SP_LISTAR_BACKUPS FOR C_QUINDIOFLIX.SP_LISTAR_BACKUPS';
    DBMS_OUTPUT.PUT_LINE('Sinonimos publicos creados/actualizados.');
...
BEGIN
    SP_BACKUP_QUINDIOFLIX(
        p_tipo_backup => 'SCHEMA',
        p_nombre_job  => 'BK_MANUAL_001',
        p_archivo     => 'MANUAL_BK_001',
        p_backup_id   => v_id,
        p_estado      => v_estado,
        p_mensaje     => v_msg
    );
