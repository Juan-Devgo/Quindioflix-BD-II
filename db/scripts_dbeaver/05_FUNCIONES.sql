...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(
        'FN_CALCULAR_MONTO - Usuario: ' || p_id_usuario ||
        ' | Meses: ' || v_meses ||
        ' | Descuento: ' || (v_descuento * 100) || '%' ||
        ' | Monto final: ' || v_monto_final
    );
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(
        'FN_CONTENIDO_RECOMENDADO - Perfil: ' || p_id_perfil ||
        ' | Genero top: ' || v_top_genero ||
        ' | Recomendacion: ' || v_titulo
    );
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('FN_CONTENIDO_RECOMENDADO - Perfil: ' || p_id_perfil || ' | Sin datos suficientes');
        RETURN NULL;
    WHEN OTHERS THEN
