...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        ROLLBACK;
        p_mensaje := 'Error: El usuario o el plan especificado no existe';
        DBMS_OUTPUT.PUT_LINE(p_mensaje);
        RAISE_APPLICATION_ERROR(-20071, 'El usuario o el plan especificado no existe');
    WHEN OTHERS THEN
        ROLLBACK;
        p_mensaje := 'Error al cambiar de plan: ' || SQLERRM;
        DBMS_OUTPUT.PUT_LINE(p_mensaje);
...
EXCEPTION
    WHEN OTHERS THEN
        p_mensaje := 'Error al generar el reporte de consumo: ' || SQLERRM;
        DBMS_OUTPUT.PUT_LINE(p_mensaje);
